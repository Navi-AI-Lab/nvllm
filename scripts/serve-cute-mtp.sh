#!/bin/bash
# nvllm -- Serve Huihui-Qwen3.6-27B-abliterated-NVFP4-MTP (text-only + MTP + 64K)
#
# Bring-up script for sakamakismile/Huihui-Qwen3.6-27B-abliterated-NVFP4-MTP on GB10.
# Text-only (vision disabled), MTP speculative decode, CUTE_PAGED attention,
...
# --limit-mm-per-prompt flags. The draft vision+MTP variant lives at
# docs/superpowers/plans/serve-cute-vision-mtp.sh.
#
...
#
# Usage:
#   ./serve-cute-mtp.sh                # bring-up defaults (n_seqs=1, MTP n=1, fusions off)
#   MAX_NUM_SEQS=2 ./serve-cute-mtp.sh # after correctness passes
#   MTP_TOKENS=3 ./serve-cute-mtp.sh   # only after n=1 stable
#   ./serve-cute-mtp.sh --debug        # eager mode, no CUDA graphs
...
# Quant flag — defaults to modelopt for the Huihui checkpoint. Set
# QUANTIZATION="" to let vLLM auto-detect (e.g. ig1/Qwen3.5-27B-NVFP4 is
# compressed-tensors, picked up automatically from config.json).
QUANTIZATION="${QUANTIZATION-modelopt}"  # single dash: empty means "no flag"; only unset triggers default
QUANT_ARGS=()
if [ -n "$QUANTIZATION" ]; then
  QUANT_ARGS+=(--quantization "$QUANTIZATION")
fi

echo "=== Launching Huihui-Qwen3.6-27B-NVFP4-MTP ($HF_MODEL) — text-only + MTP + CUTE_PAGED ==="
echo "  Model:       $HF_MODEL"
...
# Tokenizer-config override for Huihui MTP NVFP4 checkpoint.
# The published tokenizer_config.json declares tokenizer_class="TokenizersBackend"
# (transformers 5.x); our image is pinned to transformers 4.57.6 and rejects it.
# Underlying tokenizer.json is unchanged — only the wrapper hint is wrong, so we
# overlay a patched config (tokenizer_class=Qwen2Tokenizer, no `backend` key).
# Disable with NVLLM_TOKENIZER_PATCH=0 once the image moves to transformers 5.x.
TOKENIZER_PATCH_MOUNT=()
if [ "${NVLLM_TOKENIZER_PATCH:-1}" = "1" ]; then
  REPO_ROOT="$(git -C "$(dirname "${BASH_SOURCE[0]}")" rev-parse --show-toplevel)"
  PATCH_FILE="$REPO_ROOT/models/qwen3.6-mtp-tokenizer-patch/tokenizer_config.json"
  HOST_SNAPSHOT_DIR="$HOME/.cache/huggingface/hub/models--sakamakismile--Huihui-Qwen3.6-27B-abliterated-NVFP4-MTP/snapshots"
  if [ -f "$PATCH_FILE" ] && [ -d "$HOST_SNAPSHOT_DIR" ]; then
    SNAPSHOT_ID="$(ls "$HOST_SNAPSHOT_DIR" | head -1)"
    if [ -n "$SNAPSHOT_ID" ]; then
      TOKENIZER_DST="/root/.cache/huggingface/hub/models--sakamakismile--Huihui-Qwen3.6-27B-abliterated-NVFP4-MTP/snapshots/$SNAPSHOT_ID/tokenizer_config.json"
