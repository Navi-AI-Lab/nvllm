...
# Tuning knobs — override via env vars for parameter sweeps without rebuilds.
_TQ_DECODE_BLOCK_N = int(os.environ.get("TQ_DECODE_BLOCK_N", "0"))  # 0 = auto
_TQ_DECODE_NUM_WARPS = int(os.environ.get("TQ_DECODE_NUM_WARPS", "4"))
_TQ_DECODE_NUM_STAGES = int(os.environ.get("TQ_DECODE_NUM_STAGES", "2"))
_TQ_POST_BLOCK_OUT = int(os.environ.get("TQ_POST_BLOCK_OUT", "32"))
_TQ_POST_BLOCK_K = int(os.environ.get("TQ_POST_BLOCK_K", "32"))
_TQ_POST_NUM_WARPS = int(os.environ.get("TQ_POST_NUM_WARPS", "4"))
_TQ_POST_NUM_STAGES = int(os.environ.get("TQ_POST_NUM_STAGES", "2"))
...
def _norm_lut(device_type: str, device_index: int | None) -> torch.Tensor:
    ...
    lut = values.to(torch.int16).view(torch.float16).to(torch.float32)
    # Sanitise: stale cache bytes can hit float16 NaN/Inf encodings.
    # Replace with 0.0 so masked-off tokens don't poison the softmax.
    lut = torch.where(torch.isfinite(lut), lut, torch.zeros_like(lut))
    return lut
...
def _unpack_fixed_indices(
    ...
    if bits == 0:
        return tl.zeros([BLOCK_N, HEAD_SIZE_PADDED], dtype=tl.int32)

    # Byte-oriented unpacking: load 2 bytes spanning each element's bits,
    # combine into uint16, shift+mask to extract the full multi-bit value.
    start_bit = offs_d[None, :] * bits
    byte_idx = base_offset + start_bit // 8
    local_bit = start_bit % 8

    lo = tl.load(
        cache_ptr + token_base[:, None] + byte_idx * stride_cache_d,
        mask=mask_d,
        other=0,
    ).to(tl.int32)
    hi = tl.load(
        cache_ptr + token_base[:, None] + (byte_idx + 1) * stride_cache_d,
        mask=mask_d,
        other=0,
    ).to(tl.int32)

    combined = lo | (hi << 8)
    return (combined >> local_bit) & ((1 << bits) - 1)
...
def turboquant_decode_attention_fwd(
    ...
    if _TQ_DECODE_BLOCK_N > 0:
        block_n = _TQ_DECODE_BLOCK_N
    else:
    ...
    _turboquant_decode_kernel[(query.shape[0], query.shape[1])](
        ...
        num_warps=_TQ_DECODE_NUM_WARPS,
        num_stages=_TQ_DECODE_NUM_STAGES,
    )
    ...
    block_out = _TQ_POST_BLOCK_OUT
    block_k = _TQ_POST_BLOCK_K
    for acc_mse, acc_qjl, mse_inverse, qjl_inverse, indices, group, dim_pad in zip(
        ...
        _turboquant_decode_postprocess_group_kernel[grid](
            ...
            BLOCK_OUT=block_out,
            BLOCK_K=block_k,
            num_warps=_TQ_POST_NUM_WARPS,
            num_stages=_TQ_POST_NUM_STAGES,
        )
