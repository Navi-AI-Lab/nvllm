...
class PhaseE_Beta_Kernel:
    ...
    def __init__(
        ...
        # --- β-coop region-timing instrumentation (Task 3) ----------------
        # Env-gated; production path leaves this False so the compile key
        # matches the existing disk-cache artifact and no extra params are
    ...
    def run_beta_coop_full(
        ...
        *,
        ...
        # Task 3: optional caller-supplied scratch for per-region timing.
        # Shape: (total_ctas, num_regions, 2) u64 — entry+exit ticks per
        # CTA per region. When None, timing is a no-op regardless of env.
        # Production path must pass None (env unset → flag False → key
        # picks the no-instrumentation cached compile artifact).
        region_timing_buf: Optional[torch.Tensor] = None,
    ) -> None:
        ...
        # Task 3: validate region_timing_buf when supplied. Production
        # path passes None and skips this block entirely (no overhead).
        if region_timing_buf is not None:
            expected_ctas = self.slice_ctas * self.num_k_tiles * nat
            assert region_timing_buf.shape == (
                expected_ctas, self._region_timing_num_regions, 2
            ), (
                f"region_timing_buf shape {region_timing_buf.shape} != "
                f"({expected_ctas}, {self._region_timing_num_regions}, 2)"
            )
            assert region_timing_buf.dtype == torch.int64, (
                f"region_timing_buf dtype must be int64 "
                f"(u64 stored as i64), got {region_timing_buf.dtype}"
            )
            assert region_timing_buf.is_contiguous()
            assert self._region_timing_enabled, (
                "region_timing_buf supplied but CUTE_BETA_REGION_TIMING "
                "was not set at __init__; recompile required (compile "
                "key changes when flag flips)."
            )
        ...
        # Task 3: ALWAYS append the two region-timing args; the inner
        # @cute.kernel takes a `region_timing_enabled: Constexpr[bool]`
        # parameter, so the timing-off compile path generates no
        # instrumentation PTX. When enabled and a buffer was supplied,
        # pass its data_ptr; otherwise pass 0 (never dereferenced under
        # constexpr-false).
        if self._region_timing_enabled and region_timing_buf is not None:
            timing_ptr_i64 = Int64(int(region_timing_buf.data_ptr()))
        else:
            timing_ptr_i64 = Int64(0)
        all_args = all_args + (
            timing_ptr_i64,
            self._region_timing_enabled,
        )
    ...
    def _coop_full_compile_key(self) -> tuple:
        ...
        return (
            ...
            # Task 3: env-gated region-timing flag MUST be in the cache key
            # so apply_disk_cache_patch (feat/cute-full-compile-cache) does
            # not return the timing-off artifact when the timing-on call
            # path supplies the extra args.
            bool(getattr(self, "_region_timing_enabled", False)),
        )
    ...
    if _CUTE_AVAILABLE:
        ...
        def _jit_launch_phase_0_to_4(
            ...
            nat: Int32,
            stream,
            # Task 3: region-timing scratch pointer (always present).
            # When region_timing_enabled is False (constexpr), the inner
            # @cute.kernel never reads/writes through this pointer, so
            # passing 0 is safe.
        ...
        def _kernel_phase_0_to_4(
            ...
            # Task 3: region-timing scratch pointer (u64 entry+exit ticks
            # per CTA per region) + constexpr gate. When the gate is
            # False, codegen emits no instrumentation PTX, so the
