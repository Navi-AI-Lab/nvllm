#!/usr/bin/env bash
# D2.2 bisection leg: 12L_3_47 arm with CUTE_PHASE_E_PATH=lite (vs Stage 2b's auto).
#
# Hypothesis under test (per DIAGNOSIS.md, post-D2.1 narrowing):
#   D2.1 (CUTE_WO_SPLIT=1) reproduced the Stage 2b collapse shape exactly,
#   ruling out the wo_split-specific plumbing. D2.2 forces the β-LITE path
#   (paged attn only; no persistent W_O / MLP-partial workspace), keeping
#   the same 12L set otherwise.
#
#   - If this run is STABLE across 5 GSM8K-50 passes, the persistent β-coop
#     workspace pattern is the substrate — lite path doesn't share it and
#     therefore doesn't collapse. Strong substrate confirmation; D2.3 then
#     becomes a corroborating control.
#   - If this run COLLAPSES like Stage 2b, the suspect lives in phase-layer
#     selection logic (or somewhere shared between coop and lite paths) —
#     diagnosis arc widens; queue D2.3 (CUTE_PHASE_E_FUSION=0) to peel off
#     the entire phase-E layer machinery.
#
# Same soak shape as Stage 2b: boots scripts/serve-cute.sh ONCE with the
# 12L β-coop layer set + lite path, runs the dispatch audit once, then runs
# GSM8K-50 N times back-to-back on the same container.
#
# Usage:
#   ./soak_d2_2_runner.sh            # 5 runs, default
#   ./soak_d2_2_runner.sh --force    # overwrite existing soak_d2_2_lite/ dir
#
# Gate D2.2: all 5 runs ≥45/50, 0 errors, container alive, no docker.log
...
DOC_DIR="$HERE"
SOAK_DIR="$DOC_DIR/soak_d2_2_lite"
EXTRACT_DISPATCH="$DOC_DIR/extract_dispatch_log.py"
GSM8K_SCRIPT="$REPO_ROOT/scripts/gsm8k_eval_50.py"

# Worktree guard (matches runner.sh / soak_runner.sh / soak_d2_1_runner.sh).
...
# D2.2 config: same 12L arm, CUTE_PHASE_E_PATH=lite (was "auto" in Stage 2b
# and D2.1). wo_split restored to 8 (base) since D2.1 eliminated it as the
# substrate; we want to be on the base wo_split for the lite-path test.
ARM="12L_3_47_d2_2_lite"
PHASE_E_LAYERS="3,7,11,15,19,23,27,31,35,39,43,47"
EXPECTED_LITE="3,7,11,15,19,23,27,31,35,39,43,47"
WO_SPLIT="8"
...
log "boot serve-cute.sh (arm=$ARM, layers=[$PHASE_E_LAYERS], path=lite, wo=$WO_SPLIT) ..."
set +e
...
# ---------------------------------------------------------------------------
# One-shot dispatch audit. Lite path dispatches use_beta_lite=true on the
# same layer set; coop_layers list will be empty, lite_layers will match
# the expected set.
# ---------------------------------------------------------------------------
log "dispatch audit (expect lite_layers=[$EXPECTED_LITE])"
set +e
docker logs "$CONTAINER" 2>&1 \
  | "$REPO_ROOT/.venv/bin/python" "$EXTRACT_DISPATCH" \
      --expect-lite-layers "$EXPECTED_LITE" \
...
# ---------------------------------------------------------------------------
# Gate D2.2 evaluation (same shape as Gate 2b).
# ---------------------------------------------------------------------------
ALL_PASS="true"
ANY_FAIL="false"
TOTAL_ERR=0
...

{
  ...
  echo "  \"gate_d2_2_pass\": $ALL_PASS"
  echo "}"
} > "$SOAK_DIR/verdict.json"

log "D2.2 soak complete (gate_d2_2_pass=$ALL_PASS)"
