#!/usr/bin/env bash
# D2.1 bisection leg: 12L_3_47 arm with CUTE_WO_SPLIT=1 (vs Stage 2b's 8).
#
# Hypothesis under test (per DIAGNOSIS.md leading suspect):
#   - If this run is STABLE across 5 GSM8K-50 passes, the suspect lives in
#     the wo_split path (pre-WO wait, W_O slot reduction, or wo_output slot
#     layout). The Stage 2b failure was the wo_split=8 reduction substrate.
#   - If this run COLLAPSES like Stage 2b, wo_split is NOT the substrate —
#     move to D2.2 (CUTE_PHASE_E_PATH=lite).
#
# Same soak shape as Stage 2b: boots scripts/serve-cute.sh ONCE with the
# 12L β-coop layer set, runs the dispatch audit once, then runs GSM8K-50
# N times back-to-back on the same container. Tears down only after all
# runs complete.
#
# Usage:
#   ./soak_d2_1_runner.sh            # 5 runs, default
#   ./soak_d2_1_runner.sh --force    # overwrite existing soak_d2_1_wo1/ dir
#
# Gate D2.1: all 5 runs ≥45/50, 0 errors, container alive, no docker.log
...
# D2.1 config: same 12L arm, wo_split=1 (was 8 in Stage 2b).
...
# ---------------------------------------------------------------------------
# Gate D2.1 evaluation (same shape as Gate 2b).
# ---------------------------------------------------------------------------
ALL_PASS="true"
ANY_FAIL="false"
TOTAL_ERR=0
...

{
  ...
  echo "  \"gate_d2_1_pass\": $ALL_PASS"
  echo "}"
} > "$SOAK_DIR/verdict.json"

log "D2.1 soak complete (gate_d2_1_pass=$ALL_PASS)"
