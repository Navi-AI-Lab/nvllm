...
| C1 | `c1_replay_proof.sh` | FULL_AND_PIECEWISE + n=1 + flag: batch_desc.cg_mode reaches FULL at L1050 dispatch | β-coop didn't capture, or runner downgraded the mode. Audit stream wiring + dispatch logs. |
...
- C1 PASS: probe log contains `batch_desc.cg_mode=CUDAGraphMode.FULL` for at least one decode call (not just config).
