...
// Winners table (auto-generated by gen_winners_header.py). Defines
// nvllm::fp4::lookup_m_mid_winner(n, k, mp2) for the (N, K) -> bucket ->
// ShortlistCfg_idx table.
#include "nvfp4_winners_table.hpp"
...
void cutlass_fp4_bf16_gemm_dispatch(torch::stable::Tensor& D,
  ...
  if (mp2 <= 16 && !nvllm_fp4_disable_streamk()) {
    ...
    // 1. env-var override (highest priority, unchanged success path)
    const char* env = std::getenv("NVLLM_FP4_GEMM_CONFIG_M256");
    if (env && env[0] != '\0') {
      int idx = atoi(env);
      if (try_run_shortlist_config<cutlass::bfloat16_t>(
      ...
      fprintf(stderr,
              "[nvllm] NVLLM_FP4_GEMM_CONFIG_M256=%d unknown; falling through to (N=%d,K=%d) table\n",
              idx, n, k);
    }

    // 2. (N, K) -> bucket -> cfg table (new)
    int tbl_idx = nvllm::fp4::lookup_m_mid_winner(n, k, static_cast<int>(mp2));
    if (tbl_idx >= 0 &&
        try_run_shortlist_config<cutlass::bfloat16_t>(
            tbl_idx, D, A, B, A_sf, B_sf, alpha, m, n, k, stream)) {
      NVLLM_NVTX_RANGE_POP();
    ...
    // 3. production default (unchanged — shapes not in the table land here)
